...
impl Hand {
    fn get_type(self: &Hand) -> HandType {
        ...
        let card_sorted = card_counts
            .iter()
            .sorted_by_key(|(_, count)| *count);
            // .inspect(|x| println!("counts {:?}", *x));
        let mut cards_vec : Vec<(&Card, usize)> = card_sorted.clone()
            .map(|(x,y)| (*x,*y))
            .collect();
        let mut len = cards_vec.len(); 

        if len > 1 {
            // remove jokers if found and add to the most frequent card
            let clone = card_sorted.clone();
            let enum_cards = clone.enumerate();
            for (index, pair) in enum_cards {
                if let (Card::Joker, count) = pair {
                    cards_vec[len - 1].1 = cards_vec[len - 1].1 + *count;
                    cards_vec.remove(index);
                    len = len-1;
                    break;
                }
            }
        ...
        let cards : Vec<usize> = cards_vec.iter().map(|(_card, count)| *count)
            .sorted()
...
impl Ord for Hand {
    fn cmp(&self, other: &Self) -> Ordering {        
        ...
        let h1 = self.cards.iter().inspect(|x| println!("h1 after type: {:?}", &x));
        let h2 = other.cards.iter().inspect(|x| println!("h2 after type: {:?}", &x));
